...
function makeRuntime({ audio: audioMode = "web", deterministic = false } = {}) {
  ...
  vm.runInContext(script + `\n;globalThis.appTest={render,renderLand,renderChart,renderMajor,renderModes,MODES,modeNotes,modeMap,currentMode,
    ...
    setInv:(s,p)=>{invSet=s;invProg=p},
    fingering,fingerTable,fingerHint,fingerBoard,FINGER,
    doMove:()=>{invStep=(invStep+1)%3;invMoved=true;renderInversions()},
    resetMove:()=>{invStep=0;invMoved=false;renderInversions()},
    setNoteHL:v=>{noteHL=v},setNoteString:v=>{noteString=v},
    setTriad:(k,t)=>{triadKind=k;triadSet=t},setMode:v=>{modeId=v},VIEWS,majorBoard,majorTab,majRoot,MAJKEYS,MAJ_PAT,
    setMajor:(k,d,a)=>{majorKey=k;majorDegrees=d;majorArrows=a},keyStrip,runNotes,runTab,
    ...
    webAudioEngine,wavEngine,freq,getEngine:()=>engine,getAudioFault:()=>audioFault,
    renderSolo,soloZone,soloRoot,buildRun,SOLOPATTERNS,saveSolo,loadSolo,playSolo,stopSolo,
    setSoloBoxes:v=>{soloBoxes=v},setSoloRun:v=>{soloRun=v},
